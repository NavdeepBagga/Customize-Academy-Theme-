...
/////////EDIT by Navdeep Bagga(admin@navdeepbagga.com)//////////////////		
			$user=wp_insert_user( array ('first_name' => $userdata['user_fname'], 'last_name' => $userdata['user_lname'], 'user_login' => $userdata['user_login'], 'user_email' => $userdata['user_email'], 'user_pass' => $userdata['user_password'] ) ) ;
			update_usermeta($user, 'license_number', $userdata['user_license'] ) ;
			update_usermeta($user, 'user_state', $userdata['user_state'] ) ;
////////////////////////////////////////////////////////////////////////
	...
	public static function recoverUser() {
		...
		check_ajax_referer( 'password_nonce', 'nonce' );
		parse_str($_POST['data'], $userdata);
		...
		if(email_exists(sanitize_email($userdata['user_email']))) {
			...
			if(!$allow || is_wp_error($allow)) {
				...
				$mail_message=str_replace('%username%', $user->user_login, self::$data['password_message']);
	...
	public static function getFacebookProfile($ID, $fields=array()) {
		if (!empty($fields['code'])) {
			$response=wp_remote_get('https://graph.facebook.com/oauth/access_token?client_id='.self::getOption('facebook_id').'&redirect_uri=&client_secret='.self::getOption('facebook_secret').'&code='.$fields['code']);	
