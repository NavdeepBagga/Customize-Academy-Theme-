...
/////////EDIT by Navdeep Bagga(admin@navdeepbagga.com)//////////////////				
				case 'complete':
					...
					$cdate_insert= mysql_query("INSERT INTO ryw_course_complete(user_ID, course_ID, course_cdate)
							VALUES ('$userid','$course_id','$date')");
                break;
////////////////////////////////////////////////////////////////////////				
				case 'uncomplete':
	...
	public static function isSubscriber() {
		$subscriptions=array_intersect_key(self::$data['course']['plans'], array_keys(self::$data['course']['subscriptions']));
...
/////////EDIT by Navdeep Bagga(admin@navdeepbagga.com)//////////////////						
	                    ...
	                    $course = mysql_fetch_array(mysql_query("SELECT course_cdate FROM ryw_course_complete WHERE user_ID = $user_id AND course_ID = '$course_id' "));
	                    $course_cdate = $course[course_cdate];
                        $certificate['text']=str_replace('%fullname%', ThemexUser::getFullName($certificate['user']), $content[1]);
						$certificate['text']=str_replace('%firstname%', get_user_meta($current_user->ID, 'first_name', true), $certificate['text']);
                        $certificate['text']=str_replace('%lastname%', get_user_meta($current_user->ID, 'last_name', true), $certificate['text']);
					    $certificate['text']=str_replace('%license_number%', get_user_meta($current_user->ID, 'license_number', true), $certificate['text']); 
	                    $certificate['text']=str_replace('%state%', get_user_meta($current_user->ID, 'user_state', true), $certificate['text']);
	                    $certificate['text']=str_replace('%username%', $current_user->user_login, $certificate['text']);
						$certificate['text']=str_replace('%email%', $current_user->user_email, $certificate['text']);
                        $certificate['text']=str_replace('%date%', $course_cdate , $certificate['text']);
